...
| **Contentless** (`content=''`, `contentless_delete=1`) + base-table folded columns | `WITHOUT ROWID` kept — **#2143's numbers stand** | **17/17** — `MATCH` for long terms, and short terms by `LIKE` on the base table's folded columns, which return the text `id` directly | unmeasured; without `contentless_unindexed=1` it costs a mapping table and a join on every `MATCH` query, since contentless columns read back `NULL` and `MATCH` returns only the FTS rowid; with it (3.47+) the `docid` reads back and the map is an optional optimisation; a pure short-term `LIKE` reads the base table, which carries the text `id`, and needs no join |
...
for external content bound directly to a rowid table; the filtered-view design, and contentless
without `contentless_unindexed=1`, join through the id↔rowid mapping instead (`… JOIN map m ON m.rowid = fts.rowid` and `c.id = m.id`), and a
...
| Schema | **none** — direct `json_extract` expressions in the custom read; no columns, no migration | **two catalogue rebuilds** for STORED folded columns (external content, or contentless leaning on them), or none with UDF-backed VIRTUAL folded columns, which the triggers and the base-table `LIKE` can read (verified callable; read cost unmeasured), or none at all with the triggers inserting `wcpos_fold(json_extract(new.data,…))` and the fallback scanning that expression on the base table (no column reads, so manual repair only; unmeasured); none for a self-owned table |
