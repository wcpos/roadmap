...
- **A side sheet is justified only when the page behind it must be referenced.** Carbon reserves slide-in panels for when the user "needs to reference the page along with the panel"; Fluent's Drawer is for "when retaining context is beneficial." Polaris removed Sheet because it "blocks other parts of the UI, forces users to switch context, and adds complexity"; Atlassian says Drawer is being deprecated in favour of Modal.
...
3. **It is the same two panes already built.** The #111 pane roles map onto the configured POS columns rather than fixed spatial positions: ledger takes the cart column wherever it is, and tender takes the products column. This is a container change: the tender checkout moves from the `(modals)` group into the `(columns)` layout, and the `(tabs)` phone route keeps the full-screen sheet. Nothing in the tender state machine changes.
