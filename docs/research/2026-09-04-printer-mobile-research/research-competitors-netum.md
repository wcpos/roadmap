...
- Native guidance: on Android, the default GATT MTU is 23 bytes (20 payload) and chunks use negotiated **MTU − 3** after `requestMTU`. On iOS, use `maximumWriteValueLength(for:)` directly for the selected write type because it already returns the maximum payload for one Core Bluetooth write. Use write-without-response for intermediate chunks, request `CONNECTION_PRIORITY_HIGH` on Android during the transfer, and send the final chunk with response (or wait for an equivalent queue-drained/backpressure signal) before reporting completion. The final acknowledgement prevented the verified 7.4 KB receipt from losing its tail (gotcha #38). ([Apple `maximumWriteValueLength(for:)`](https://developer.apple.com/documentation/corebluetooth/cbperipheral/maximumwritevaluelength(for:)); [Reliable BLE Data Transfer](https://uynguyen.github.io/2026/04/12/Reliable-BLE-Data-Transfer-MTU-Throughput-Chunking/); [react-native-ble-plx MTU wiki](https://github.com/dotintent/react-native-ble-plx/wiki/MTU-Negotiation)).
...
- **iOS: BLE is the evidenced path; MFi is UNVERIFIED.** Netum's FAQ PDF states the printer *"can not connect with the device via it's built- in Bluetooth, must be connected through an application/software as a mediator"* and separately says it does not support AirPrint. A Newegg listing says it cannot be discovered through the device's built-in Bluetooth UI. No MFi claim was found, but vendor silence and lack of AirPrint do not prove that ExternalAccessory is unavailable. Treat the printer as BLE-only until Netum confirms its accessory protocols or a physical iOS test verifies whether it appears in Settings.
...
- **OS-level pairing is UNVERIFIED** — current vendor/listing evidence directs the merchant to connect from inside an app, but do not rule out a Settings/MFi path until Netum confirms its accessory protocols or a physical iOS test does (see (j)).
...
7. **Netum-class printers need a wholly different wizard branch from Epson**: current evidence supports in-app BLE GATT on iOS and no AirPrint, while OS-level pairing/MFi remains unverified. Direct merchants to the in-app BLE flow unless vendor confirmation or a physical iOS test establishes a Settings/ExternalAccessory path.
8. **Probe UUIDs in order 18F0/2AF1 → FF00/FF02 → 49535343-…**; on Android use 20-byte chunks until MTU negotiation succeeds, then MTU−3; on iOS use `maximumWriteValueLength(for:)` directly for the selected write type. Keep a small inter-chunk delay and prefer write-without-response for intermediate chunks, but acknowledge the final chunk (or wait for an equivalent queue-drained/backpressure signal) before reporting completion so the receipt tail is not lost (#38).
