...
- Native guidance: default GATT MTU is 23 bytes (20 payload); after negotiation (`requestMTU` on Android, `maximumWriteValueLength` on iOS) chunk at **MTU − 3**, use write-without-response for intermediate chunks, request `CONNECTION_PRIORITY_HIGH` during the transfer, and send the final chunk with response (or wait for an equivalent queue-drained/backpressure signal) before reporting completion. The final acknowledgement prevented the verified 7.4 KB receipt from losing its tail (gotcha #38). ([Reliable BLE Data Transfer](https://uynguyen.github.io/2026/04/12/Reliable-BLE-Data-Transfer-MTU-Throughput-Chunking/); [react-native-ble-plx MTU wiki](https://github.com/dotintent/react-native-ble-plx/wiki/MTU-Negotiation)).
...
8. **Probe UUIDs in order 18F0/2AF1 → FF00/FF02 → 49535343-…**; chunk at 20 bytes unless MTU negotiation succeeds, then MTU−3 with a small inter-chunk delay. Prefer write-without-response for intermediate chunks, but acknowledge the final chunk (or wait for an equivalent queue-drained/backpressure signal) before reporting completion so the receipt tail is not lost (#38).
