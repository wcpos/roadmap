...
- **The roadmap main clone (`~/Projects/roadmap`) cannot be pulled right now.** It is 78 commits behind and dirty with another session's printer-research work, and a fast-forward would touch that work: four of its modified or staged files (`docs/handoffs/2026-09-04-printer-mobile-preflight.md` and three under `docs/research/2026-09-04-printer-mobile-research/`) are also changed by incoming commits, and the untracked `docs/handoffs/2026-09-05-printer-setup-cashier-map.md` collides with an incoming tracked file. Most likely the same content already landed via roadmap#147 and the local copies are leftovers, but that is a by-hand check, not something a session should resolve by stashing or deleting. Until it is reconciled, **this doc is readable at the worktree path** `/Users/kilbot/Projects/roadmap-worktrees/docs-flexsearch-handoff/docs/handoffs/2026-09-14-flexsearch-churn-followups.md`, or on GitHub in roadmap#310 / `main` after merge. The release session's handoff (`2026-09-14-post-1.10.13-outstanding.md`) was also sitting in that clone **untracked**; this PR carries it in with its corrections, so after the merge that untracked copy is a second collision to delete before pulling.

## Opening prompt for the next session

```
Read /Users/kilbot/Projects/roadmap-worktrees/docs-flexsearch-handoff/docs/handoffs/2026-09-14-flexsearch-churn-followups.md (canonical copy: docs/handoffs/ on wcpos/roadmap main once the main clone is reconciled, see item 7) and do item 1 on the main lane: the comment at the pre-write persisted marking in scripts/patch-rxdb-premium-flexsearch-churn.mjs, plus the RxPipeline no-retry pin test if it fits. Worktree from origin/main, PR to main, keep the two lanes byte-identical via the routine main→next sync, and mutation-check the new test. Then file the tracking issue for item 2 and stop; the lane for item 2 is my call.
```
