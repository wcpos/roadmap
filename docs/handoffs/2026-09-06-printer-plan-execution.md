...
| wcpos/electron#411 | USB rows named by the product string; `usb-query-model` (GS I 67) | open; CI re-run after the preload allowlist fix |
...
5. **Status query on raw lanes** (agent in flight at handoff time): paper-out / cover-open after a test page over BLE goes straight to the paper line.
