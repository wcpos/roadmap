...
| (in #1884, 1f785e4627) | **Bluetooth Classic (SPP) on Android** — Paul's go-ahead 2026-09-06 ("go ahead with 1"); a local Expo module `apps/main/modules/bluetooth-spp` rather than the RC-only `react-native-bluetooth-classic`; paired list → cards, RFCOMM lane, `DLE EOT` status, one card per dual-mode printer; roadmap#136 #45 | pushed; **dev-client rebuild still to do** — EAS `b90d5103` errored (frozen lockfile: the dev worktree carries the Electron submodule, which the lockfile has never listed; build from a worktree without submodule contents, e.g. `codex-spec-m1`), EAS `d1e83fbe` from that worktree was cancelled at 09:07 after 28 min; the month is already at 38 development builds against CI's ceiling of 20, so the next trigger is Paul's |
...
5. **Bluetooth Classic on Android** (1f785e4627, needs a fresh Android dev client — none built yet, see the table): pair the Netum in Android Settings, switch its LE off if it has such a mode (or use any SPP-only printer) — the card should read "Bluetooth" from the paired list, print through the socket, and an unpaired printer should show the *not paired* line with **Open Bluetooth settings**. The Netum is dual-mode, so with LE on it lands on its LE card and the SPP row stays hidden (log line `Bluetooth SPP twin hidden`).
6. **Status query on the Bluetooth LE lanes** (e4e8277915): after a test page the app asks the printer `DLE EOT 1/2/4` over the 18F0/2AF0 notify; paper-out or cover-open goes straight to the paper line instead of asking. Needs a real Netum paper-out run. USB and raw TCP through Electron main still cannot ask (`usb-query-status` is the named follow-up in wcpos/electron).

## Decisions still Paul's

- ~~Bluetooth Classic (SPP) on phones~~ — decided 2026-09-06, built the same day (see above).
...
- `#40` scan read as Wi-Fi-only · `#41` BLE "no longer in range" and the forgotten device · `#42` ruler answer silently cycled widths · `#43` chooser lists the whole shop floor · `#44` Electron USB rows never carried the product string · `#45` Android never listed a Bluetooth Classic printer.

## Final counts at handoff

printer package vitest 67 files / 657 tests; core `settings/print` 24 suites / 190 tests; virtual-printer 41 node tests; utils logger 96. #1884 head e4e8277915; dev build merge 5e21420cde.
