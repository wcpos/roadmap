...
| Component | Verdict | Split | Size | States | Ledger | Note |
|---|---|---|---|---|---|---|
| `button` | restyle | none | ctl, tile for `.xl` | Loading (spinner) | 13 | Heights to the floored `ctl` token in the control core pass; `.q` is `ghost-quiet` for text-only actions and `ghost` for icon buttons (Q1). The `compact` size retires (#289) → removal R4, ruled: `compact` and line 5 struck, the one caller and both tests move to `sm`. The 40-variant catalogue is audited in the token pass; any pruning is its own removal ticket. `ButtonPill` moves to `chip` (below). |
...
| `pos/cart/cells` | rebuild | pointer | 13 | The `×` goes; removal is **one route**: the swipe strip's Remove on touch, the hover action's Remove with a mouse, both the same control calling `pulseRemove`, which keeps sole ownership of the re-entrancy latch (lines 1–3 preserved; Q17, §7a). The keypad's `−` stops at 1 and dims; it never removes. Name and price edit in place (`editable-field`). Quantity's fast path on the row is the `keypad` popover on the quantity (decision 36); the Edit dialog keeps its quantity field, like name and price (R6 withdrawn 2026-09-18). Lines 4–5 preserved on the keypad's display. |
| `pos/cart/cells/edit-*`, `pos/cart/totals` | restyle | — | 4, 4, 2, 3 | Edit line item stays the complete form of the line (name, price, quantity, tax class, meta); nothing leaves it (R6 withdrawn 2026-09-18). The note row stays (decision 44). |
...
| `components/product` | restyle | — | 50 | `variable-product-row.tsx` (the inline drill-in) stays as an option behind the products setting (R7 withdrawn 2026-09-18); every line stays live. |
...
The [removal gate](2026-09-18-library-strategy.md#6-the-removal-gate): nothing below is struck until its ticket closes with Paul's ruling, and each lands in its own PR. **Ruled 2026-09-18 on [Rule on the removal list from the component map](https://github.com/wcpos/roadmap/issues/351)**; the Ruling column is the outcome, the resolution comment there holds the evidence. Standing rule set in that grilling: **the mockups guide the design only; they are not the source of truth for forms, fields or features. Follow what is in the application today, and remove a feature or field only with a very, very good reason.** Already ruled on #340 and not repeated here: the `toggle`, `tree-select` and `data-table` folders and their lines; the tokens `--tertiary`, `--duration-750`, `--error`.

| # | Removal | Reason | Ruling (2026-09-18) |
|---|---|---|---|
| R1 | `pos/checkout/payments/server/LEDGER.md` line 12 (claim capture narration from either foreground or background) | `70a2d4b086` made the terminal-payments service the sole narrator; a migration following line 12 could reintroduce two writers. Strike, and add the single-narrator invariant under the next number. | **Strike.** Line 12 struck; line 19 states the single-narrator invariant, evidenced by the service test "narrates once across duplicate/stale delivery and remounts" and lines 16–18. |
| R2 | `pos/contexts/overlay-side/LEDGER.md` line 1's phone clause ("bottom sheets on phones") and its column-position derivation | The subject rule (decision 46) generalises lines 1–2; under 640 a side panel is a page (#290). The rest of both lines is preserved. | **Strike lines 1 and 2 whole** (a clause cannot be struck; lines are never reworded). Lines 3 and 4 state the subject rule and the page-under-640 rule, citing the old commits as the origin of what survives. |
| R3 | `status-badge`'s tinted filled pill shape | Filled pills are on the direction's rejected list; the badge is a dot + word everywhere (#287). Line 1 preserved. | **Strike.** No shape prop; the restyle PR is the removal PR, line 1 preserved. |
| R4 | `button`'s `compact` size variant | Same hit area as `sm` (`h-9 px-3`) with a smaller label (`text-xs` vs `text-sm`), and collides with the scale step's name (#289). | **Strike `compact` and strike line 5** (its substance is the compact pairing; lines are not reworded). The one caller, the notification panel's mark-all-read, moves to `sm`; the two tests repoint at `sm` so the touch-safe hit area stays pinned. |
| R5 | `components/header`: lines 1–4 (measured title centring, transparent-until-measured) and 5 (light system-bar icons against the dark sidebar), line 22 (a random upgrade string), and the folder at promotion | The drawn page bar is title-left with no centring; the rail is pale; the strip shows one string (decision 43). Lines 6–21 and 23 rehome first. | **As filed, three PRs in order:** the rehome (lines 6–21 and 23 to the cashier and bell panels, line 20 repointed at `sm` per R4; adds a line on the rail's ledger that the status-bar style follows the rail's surface), the strikes (1–5 and 22), the folder at promotion. |
| R6 | the quantity field of `pos/cart/cells/edit-line-item` | One place to change a quantity: the keypad on the line (§7a). Any of its four lines that names quantity is listed on the ticket. | **Withdrawn.** Name and price are also editable on the row and in the panel; the panel is the complete form of the line and the row carries fast paths, both through one update. The one-way ruling is about idioms, not a value reachable from two surfaces. Nothing removed. |
| R7 | `components/product/variable-product-row.tsx`, the inline drill-in, and the products setting that chose it | One drill-in idiom: the breadcrumb and the pane (§7a); reverses decision 38's "both, as a setting". The row's ledger lines (in `components/product`'s 50) are listed on the ticket. | **Withdrawn; decision 38 stands.** The breadcrumb drill-in is the **default**; the grid's variations popover and the table's inline expansion stay as options behind the products setting for people who want the old style. All of `components/product`'s lines stay live. |
| R8 | the `modal` folder, at promotion of `v2/dialog` | One overlay component (§7a); its 12 lines are carried by `v2/dialog`'s ledger as preserved or n/a before deletion. | **As filed, gated:** deleted at promotion only after every one of `modal`'s 12 lines is preserved with a pointer or n/a in `v2/dialog`'s ledger; the five with no `dialog` twin (1, 3, 5, 6, 10) need their test or pointer first. |
| R9 | the `toggle-group` folder, after its six callers move to `segmented-control` | One value picker (§7a); its one line is carried or struck on the ticket. | **As filed.** All six callers are single-select; the one line is carried onto `segmented-control`'s ledger as its origin; the folder goes in its own PR after the sixth caller moves. |
| R10 | `error-boundary` line 3 ("reports every render exception … once"), surfaced by the refresh | `ccd5b3706c` skips an error already reported under its own code; line 4 records the exception. | **Keep.** Lines 3 and 4 read as one rule; nothing reversed, nothing struck. |

### 7a. One way to do each thing

Paul, 2026-09-18, on this ticket: where two ways to do one thing had crept in, one goes. The five found: removing a cart line (the strip's Remove, not the keypad's `−`), ~~changing a quantity (the keypad, not the Edit dialog)~~ and ~~drilling into a variable product (the pane, not the inline rows)~~ (both withdrawn on #351, 2026-09-18: the one-way rule is about idioms, not about a shipped field or feature; see R6 and R7), the side panel (`dialog`, not `modal`), and the few-segment value picker (`segmented-control`, not `toggle-group`). Considered and kept apart because they are two things: the cart's order strip and the `tabs` component (switching orders is not switching views). Every future contract carries the test: if a second route to the same outcome appears in the drawing or the code, the contract names the one that stays and files the other under §7.

The nine plus R10 were ruled on one ticket, [Rule on the removal list from the component map](https://github.com/wcpos/roadmap/issues/351), on 2026-09-18 after the refresh ([#350](https://github.com/wcpos/roadmap/issues/350)); the Ruling column above records each. Flagged for the refresh pass, not yet a removal: `components/order`'s tap-to-filter on a coarse pointer, `tabs` line 9 once the last `asSelect` caller switches, `toggle-group` if its callers reach zero.
